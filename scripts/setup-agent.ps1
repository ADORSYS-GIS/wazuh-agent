...
function Install-WazuhAgent {
    ...
    $process = Start-Process -FilePath "powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -File `"$InstallerPath`" -WAZUH_AGENT_VERSION `"$WAZUH_AGENT_VERSION`" -WAZUH_MANAGER `"$WAZUH_MANAGER`"" -NoNewWindow -PassThru -RedirectStandardOutput "$env:TEMP\wazuh_output.log" -RedirectStandardError "$env:TEMP\wazuh_error.log" -Wait
