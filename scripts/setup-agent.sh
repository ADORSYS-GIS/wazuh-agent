...
if [ "$(uname)" = "Linux" ]; then
    ...
    WAZUH_CERT_OAUTH2_CLIENT_URL="https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-cert-oauth2/refs/tags/v$WOPS_VERSION/scripts/linux/install.sh"
    ...
    WAZUH_CERT_OAUTH2_CLIENT_URL="https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-cert-oauth2/refs/tags/v$WOPS_VERSION/scripts/macos/install.sh"
...
curl -SL -s "$WAZUH_CERT_OAUTH2_CLIENT_URL" > "$TMP_FOLDER/install-wazuh-cert-oauth2.sh"
...
if ! (maybe_sudo env bash "$TMP_FOLDER/setup-docker.sh" < /dev/null) 2>&1; then
