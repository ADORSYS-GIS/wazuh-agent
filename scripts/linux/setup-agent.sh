...
USER=${USER:-"root"}
GROUP=${GROUP:-"wazuh"}
LINUX_SCRIPT_PATH="scripts/linux/install.sh"
...
while getopts "s:ntbh" opt; do
    case ${opt} in
        ...
        \?)
            ...
            ;;
        :)
            error_message "Option -$OPTARG requires an argument." >&2
            help_message
            exit 1
            ;;
        *)
...
if [ "$IDS_ENGINE" = "$SURICATA_ENGINE" ] && [ -z "$SURICATA_MODE" ]; then
...
download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-cert-oauth2/${WAZUH_CERT_OAUTH2_REPO_REF}/${LINUX_SCRIPT_PATH}" "$TMP_FOLDER/install-wazuh-cert-oauth2.sh" "${LINUX_SCRIPT_PATH}" "install-wazuh-cert-oauth2.sh" "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-cert-oauth2/${WAZUH_CERT_OAUTH2_REPO_REF}/checksums.sha256"

download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-agent-status/${WAZUH_AGENT_STATUS_REPO_REF}/${LINUX_SCRIPT_PATH}" "$TMP_FOLDER/install-wazuh-agent-status.sh" "${LINUX_SCRIPT_PATH}" "install-wazuh-agent-status.sh" "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-agent-status/${WAZUH_AGENT_STATUS_REPO_REF}/checksums.sha256"

download_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-yara/${WAZUH_YARA_REPO_REF}/scripts/linux/install.sh" "$TMP_FOLDER/install-yara.sh" "Yara install script"
...
# Safeguard: If YARA deb package is installed but rules are missing (half-installed state), remove it
if command_exists dpkg && dpkg -s yara >/dev/null 2>&1; then
    if [ ! -f "/var/ossec/active-response/bin/yara.sh" ] || [ ! -f "/var/ossec/ruleset/yara/rules/yara_rules.yar" ]; then
        warn_message "Found half-installed YARA state. Cleaning up..."
        maybe_sudo apt-get remove -y yara >/dev/null 2>&1 || true
    fi
fi
...
if [ "$IDS_ENGINE" = "$SURICATA_ENGINE" ]; then
    ...
    download_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-suricata/${WAZUH_SURICATA_REPO_REF}/scripts/linux/install.sh" "$TMP_FOLDER/install-suricata.sh" "Suricata install script"
    ...
    download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-snort/${WAZUH_SNORT_REPO_REF}/${LINUX_SCRIPT_PATH}" "$TMP_FOLDER/install-snort.sh" "${LINUX_SCRIPT_PATH}" "install-snort.sh" "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-snort/${WAZUH_SNORT_REPO_REF}/checksums.sha256"
...
if [ "$INSTALL_TRIVY" = "TRUE" ]; then
    ...
    download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-trivy/${WAZUH_TRIVY_REPO_REF}/${LINUX_SCRIPT_PATH}" "$TMP_FOLDER/install-trivy.sh" "${LINUX_SCRIPT_PATH}" "install-trivy.sh" "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-trivy/${WAZUH_TRIVY_REPO_REF}/checksums.sha256"
...
if ! download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-agent/$WAZUH_AGENT_REPO_REF/scripts/linux/ca-install.sh" "$TMP_FOLDER/ca-install.sh" "scripts/linux/ca-install.sh" "Linux CA installer"; then
