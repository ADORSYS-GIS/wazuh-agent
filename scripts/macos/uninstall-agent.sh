...
if [ "$UNINSTALL_TRIVY" = "TRUE" ]; then
    download_and_verify_file "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-trivy/${WAZUH_TRIVY_REPO_REF}/${MACOS_SCRIPT_PATH}" "$TMP_FOLDER/uninstall-trivy.sh" "${MACOS_SCRIPT_PATH}" "Trivy uninstall script" "https://raw.githubusercontent.com/ADORSYS-GIS/wazuh-trivy/${WAZUH_TRIVY_REPO_REF}/checksums.sha256"
fi
...
if [ "$UNINSTALL_VELOCIRAPTOR" = "TRUE" ]; then
    ...
    # Unload the launchd daemon if it exists
    if maybe_sudo launchctl list | grep -q "com.velocidex.velociraptor" 2>/dev/null; then
        maybe_sudo launchctl unload -w "$vr_plist_file" || warn_message "Failed to unload Velociraptor daemon; continuing"
    fi

    # Remove the plist file
    if [ -f "$vr_plist_file" ]; then
        maybe_sudo rm -f "$vr_plist_file"
        info_message "Velociraptor plist file removed."
    fi
